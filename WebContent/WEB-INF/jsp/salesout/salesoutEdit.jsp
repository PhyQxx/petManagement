...
<script type="text/javascript">
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editSalesout.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "salesoutController/updateSalesout";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "salesoutController/insertSalesout";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editSalesout.id}">
			<div class="form-group">
				<label for="orderno" class="col-sm-3 col-xs-3 control-label">订单编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="orderno" name="orderno" value="${editSalesout.orderno}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="goods" class="col-sm-3 col-xs-3 control-label">出库商品</label>
				<div class="col-sm-8 col-xs-8">
					<input id="goods" name="goods" value="${editSalesout.goods}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="amount" class="col-sm-3 col-xs-3 control-label">商品数量</label>
				<div class="col-sm-8 col-xs-8">
					<input id="amount" name="amount" value="${editSalesout.amount}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editSalesout.remark}"
						class="form-control" type="text" />
				</div>
			</div>
