		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'salesoutController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'salesoutController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'salesoutController/deleteSalesout',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
									Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'salesoutController/selectSalesoutList',//查询数据的地址
		...
		<div class="widget-body">
			<div class="widget-control">
				<form id="selectFrom" class="widget-form" method="post">
					...
					<div class="widget-screen">
						<label for="goods" class="control-label">出库商品</label>
						<div class="widget-item">
							<input class="form-control" id="goods"
								name="goods" type="text" placeholder="可按出库商品模糊查询">
						</div>
					</div>
