...
<script type="text/javascript">
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editActivity.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "activityController/updateActivity";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "activityController/insertActivity";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editActivity.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">活动编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editActivity.no}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="name" class="col-sm-3 col-xs-3 control-label">活动名称</label>
				<div class="col-sm-8 col-xs-8">
					<input id="name" name="name" value="${editActivity.name}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="content" class="col-sm-3 col-xs-3 control-label">活动内容</label>
				<div class="col-sm-8 col-xs-8">
					<input id="content" name="content" value="${editActivity.content}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editActivity.remark}"
						class="form-control" type="text" />
				</div>
			</div>
