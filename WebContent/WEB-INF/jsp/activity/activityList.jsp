		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'activityController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'activityController/openEditPage?id=' + rows[0].id })
					}
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'activityController/selectActivityList',//查询数据的地址
		form : 'selectFrom',//用于筛选的form id
		//字段
		columns : [ 
			{ field : 'id', title : 'ID', width : 100 , hidden:true}, 
			{ field : 'no', title : '活动编号', width : 100 }, 
					...
					<div class="widget-screen">
						...
						<div class="widget-item">
							<input class="form-control" id="name" name="name"
								type="text" placeholder="可按活动名称模糊查询">
						</div>
					</div>
