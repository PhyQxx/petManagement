...
<script type="text/javascript">
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "workController/updateWork";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "workController/insertWork";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editWork.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">工作编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editWork.no}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="name" class="col-sm-3 col-xs-3 control-label">工作名称</label>
				<div class="col-sm-8 col-xs-8">
					<input id="name" name="name" value="${editWork.name}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="content" class="col-sm-3 col-xs-3 control-label">工作内容</label>
				<div class="col-sm-8 col-xs-8">
					<input id="content" name="content" value="${editWork.content}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editWork.remark}"
						class="form-control" type="text" />
				</div>
			</div>
