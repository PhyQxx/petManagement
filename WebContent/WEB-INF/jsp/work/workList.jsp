		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加工作', url : 'workController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑工作', url : 'workController/openEditPage?id=' + rows[0].id })
					}
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'workController/selectWorkList',//查询数据的地址
		form : 'selectFrom',//用于筛选的form id
		//字段
		columns : [ 
			{ field : 'id', title : 'ID', width : 100 , hidden:true}, 
			{ field : 'no', title : '工作编号', width : 100 }, 
			{ field : 'name', title : '工作名称', width : 100 }, 
			{ field : 'content', title : '工作内容', width : 100 }, 
...
<body>
	<div class="widget">
		<div class="widget-header">
			<span class="title">员工工作管理</span>
		</div>
		<div class="widget-body">
			<div class="widget-control">
				<form id="selectFrom" class="widget-form" method="post">
					<div class="widget-screen">
						<label for="name" class="control-label">工作名称</label>
						<div class="widget-item">
							<input class="form-control" id="name" name="name"
								type="text" placeholder="可按工作名称模糊查询">
						</div>
					</div>
