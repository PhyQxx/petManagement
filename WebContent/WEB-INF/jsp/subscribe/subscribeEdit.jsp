...
<head>
<base href="<%=basePath%>">
<title>客户预约提醒编辑</title>
<jsp:include page="/WEB-INF/jsp/login/include.jsp"></jsp:include>
<script type="text/javascript">
$(document).ready(function() {
	// 默认初始化
	$("#way").comboBox();
	var way = "${editSubscribe.way}";

	// 修改页面 填充状态指定项   
	if (way) {
		$("#way").comboBox('select', way);
	}
})
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editSubscribe.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "subscribeController/updateSubscribe";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "subscribeController/insertSubscribe";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editSubscribe.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">客户编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editSubscribe.no}"
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="name" name="name" value="${editSubscribe.name}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="phone" name="phone" value="${editSubscribe.phone}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="time" class="col-sm-3 col-xs-3 control-label">提醒时间</label>
				<div class="col-sm-8 col-xs-8">
					<input id="time" name="time" value="${editSales.time}"
						class="form-control" type="date" />
				</div>
			</div>
			<div class="form-group">
				<label for="content" class="col-sm-3 col-xs-3 control-label">提醒内容</label>
				<div class="col-sm-8 col-xs-8">
					<input id="content" name="content" value="${editSubscribe.content}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="way" class="col-sm-3 col-xs-3 control-label">提醒方式</label>
				<div class="col-sm-8 col-xs-8">
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editSubscribe.remark}"
						class="form-control" type="text" />
				</div>
			</div>
