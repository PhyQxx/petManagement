...
<script type="text/javascript">
	...
	$(document).ready(function() {
		// 默认初始化
		$("#types").comboBox();
		var types = "${editReceivable.types}";

		// 修改页面 填充状态指定项   
		if (types) {
			$("#types").comboBox('select', types);
		}
	})
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editReceivable.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "receivableController/updateReceivable";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "receivableController/insertReceivable";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editReceivable.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">订单编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editReceivable.no}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="price" class="col-sm-3 col-xs-3 control-label">应付金额</label>
				<div class="col-sm-8 col-xs-8">
					<input id="price" name="price" value="${editReceivable.price}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="types" class="col-sm-3 col-xs-3 control-label">付款状态</label>
				<div class="col-sm-8 col-xs-8">
			...
			<div class="form-group">
				<label for="payer" class="col-sm-3 col-xs-3 control-label">收款人</label>
				<div class="col-sm-8 col-xs-8">
					<input id="payer" name="payer" value="${editReceivable.payer}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editReceivable.remark}"
						class="form-control" type="text" />
				</div>
			</div>
