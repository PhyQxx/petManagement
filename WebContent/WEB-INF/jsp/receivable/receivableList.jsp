		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'receivableController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'receivableController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'receivableController/deleteReceivable',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
									Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'receivableController/selectReceivableList',//查询数据的地址
		form : 'selectFrom',//用于筛选的form id
		//字段
		columns : [ 
			{ field : 'id', title : 'ID', width : 100 , hidden:true}, 
			{ field : 'no', title : '订单编号', width : 100 }, 
			{ field : 'price', title : '应收金额', width : 100 }, 
			{ field : 'types', title : '收款状态', width : 100 }, 
			{ field : 'payer', title : '收款人', width : 100 }, 
		...
		<div class="widget-body">
			<div class="widget-control">
				<form id="selectFrom" class="widget-form" method="post">
					...
					<div class="widget-screen">
						<label for="payer" class="control-label">收款人</label>
						<div class="widget-item">
							<input class="form-control" id="payer"
								name="payer" type="text" placeholder="可按收款人模糊查询">
						</div>
					</div>
