...
<script type="text/javascript">
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editCuring.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "curingController/updateCuring";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "curingController/insertCuring";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editCuring.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">宠物编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editCuring.no}"
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="name" name="name" value="${editCuring.name}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="phone" name="phone" value="${editCuring.phone}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="weight" class="col-sm-3 col-xs-3 control-label">宠物体重</label>
				<div class="col-sm-8 col-xs-8">
					<input id="weight" name="weight" value="${editCuring.weight}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="types" class="col-sm-3 col-xs-3 control-label">养护类型</label>
				<div class="col-sm-8 col-xs-8">
					<select class="form-control" id="types" name="types">
						<option selected="selected" value="000">请选择</option>
						<option value="0">生育</option>
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editCuring.remark}"
						class="form-control" type="text" />
				</div>
			</div>
