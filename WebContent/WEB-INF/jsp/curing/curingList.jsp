		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'curingController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'curingController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'curingController/deleteCuring',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
									Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'curingController/selectCuringList',//查询数据的地址
		...
		columns : [ 
			...
			{ field : 'weight', title : '宠物体重', width : 100 }, 
			{ field : 'types', title : '养护类型', width : 100 }, 
...
<body>
	<div class="widget">
		<div class="widget-header">
			<span class="title">待产养护信息管理</span>
		</div>
