		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'salesController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'salesController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'salesController/deleteSales',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
								Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'salesController/selectSalesList',//查询数据的地址
		form : 'selectFrom',//用于筛选的form id
		//字段
		columns : [ 
			{ field : 'id', title : 'ID', width : 100 , hidden:true}, 
			{ field : 'breed', title : '宠物品种', width : 100 }, 
			{ field : 'sex', title : '宠物性别', width : 100 }, 
			{ field : 'birthday', title : '宠物生日', width : 100 }, 
			{ field : 'price', title : '宠物价格', width : 100 }, 
			{ field : 'time', title : '出售时间', width : 100 }, 
			{ field : 'buyer', title : '购买人', width : 100 }, 
			{ field : 'seller', title : '出售人', width : 100 }, 
...
<body>
	<div class="widget">
		<div class="widget-header">
			<span class="title">活体出售信息管理</span>
		</div>
		<div class="widget-body">
			<div class="widget-control">
				<form id="selectFrom" class="widget-form" method="post">
					<div class="widget-screen">
						<label for="buyer" class="control-label">购买人</label>
						<div class="widget-item">
							<input class="form-control" id="buyer" name="buyer"
								type="text" placeholder="可按购买人模糊查询">
						</div>
					</div>
					<div class="widget-screen">
						<label for="userName" class="control-label">出售人</label>
						<div class="widget-item">
							<input class="form-control" id="userName"
								name="userName" type="text" placeholder="可按出售人名称模糊查询">
						</div>
					</div>
