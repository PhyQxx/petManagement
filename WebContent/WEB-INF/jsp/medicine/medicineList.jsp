		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'medicineController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'medicineController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'medicineController/deleteMedicine',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
									Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'medicineController/selectMedicineList',//查询数据的地址
		...
		columns : [ 
			...
			{ field : 'bedno', title : '床位号', width : 100 }, 
...
<body>
	<div class="widget">
		<div class="widget-header">
			<span class="title">医疗信息管理</span>
		</div>
		<div class="widget-body">
			<div class="widget-control">
				<form id="selectFrom" class="widget-form" method="post">
					<div class="widget-screen">
						<label for="name" class="control-label">宠物姓名</label>
						<div class="widget-item">
							<input class="form-control" id="name" name="name"
								type="text" placeholder="可按宠物姓名模糊查询">
						</div>
					</div>
					<div class="widget-screen">
					...
					<div class="widget-screen">
						<label for="bedno" class="control-label">床位号</label>
						<div class="widget-item">
							<input class="form-control" id="bedno"
								name="bedno" type="text" placeholder="可按床位号查询">
						</div>
					</div>
