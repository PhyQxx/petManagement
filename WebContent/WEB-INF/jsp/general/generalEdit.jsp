...
<script type="text/javascript">
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editGeneral.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "generalController/updateGeneral";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "generalController/insertGeneral";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editGeneral.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">账单编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editGeneral.no}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				<label for="types" class="col-sm-3 col-xs-3 control-label">账单类型</label>
				<div class="col-sm-8 col-xs-8">
					<select class="form-control" id="types" name="types">
						<option selected="selected" value="000">请选择</option>
						<option value="0">公司交易</option>
						<option value="1">个人交易</option>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editGeneral.remark}"
						class="form-control" type="text" />
				</div>
			</div>
