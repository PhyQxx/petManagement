		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加员工薪资', url : 'moneyController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑员工薪资', url : 'moneyController/openEditPage?id=' + rows[0].id })
					}
...
// 					var editUserIds= rows[0].userId;
				...
				alertify.confirm("确认删除吗?", 
					function(e) {
						if (e) {
							$.ajax({
								url : 'moneyController/deleteMoney',
								type : 'post',//提交的方式
								dataType : 'json',
								data : {
									Ids : userIds
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'moneyController/selectMoneyList',//查询数据的地址
		form : 'selectFrom',//用于筛选的form id
		//字段
		columns : [ 
			{ field : 'id', title : 'ID', width : 100 , hidden:true}, 
			{ field : 'no', title : '员工编号', width : 100 }, 
			{ field : 'name', title : '员工姓名', width : 100 }, 
			{ field : 'content', title : '员工薪资', width : 100 }, 
// 			{ field : 'time', title : '入职时间', width : 100 }, 
					...
					<div class="widget-screen">
						...
						<div class="widget-item">
							<input class="form-control" id="name" name="name"
								type="text" placeholder="可按员工姓名模糊查询">
						</div>
					</div>
