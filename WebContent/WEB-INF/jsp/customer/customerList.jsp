		...
		var toolbar =[ 
			{ text : '新增', validCount : '0+', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '添加记录', url : 'customerController/openEditPage' })
				} 
			}, { text : '修改', validCount : '1', 
				handler : function(rows) {
					modal.open({width:'600px',height:'500px', title : '编辑记录', url : 'customerController/openEditPage?id=' + rows[0].id })
					}
...
// 		initToolBarByFuncPower(toolbar,"${funcPowers}");
		// datagrid 表格配置
		$('#table').datagrid({
		url : 'customerController/selectCustomerList',//查询数据的地址
		...
		columns : [ 
			...
			{ field : 'petno', title : '宠物编号', width : 100 }, 
			{ field : 'member', title : '是否是会员', width : 100 }, 
