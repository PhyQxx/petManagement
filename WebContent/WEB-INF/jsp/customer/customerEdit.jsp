...
<script type="text/javascript">
	$(document).ready(function() {
		// 默认初始化
		$("#member").comboBox();
		var member = "${editCustomer.member}";

		// 修改页面 填充状态指定项   
		if (member) {
			$("#member").comboBox('select', member);
		}
	})
	...
	$(function() {
		...
		// 	var userRoleId = "${editUser.userRoleId}";//获取编辑用户的角色ID
		var Id = "${editCustomer.id}";//获取编辑用户用户ID
		// 	$("#userRoleId").comboBox();
		// 	if(userRoleId!=""){
	...
	function save() {
		...
		if ($.valid()) {
			// 默认为修改页面
			var urlAddress = "customerController/updateCustomer";
			// 如果没有获取到id值 则为新增页
			if ($('#hidId').val() == "") {
				urlAddress = "customerController/insertCustomer";
			}
			//提交保存
			$("#editForm").ajaxSubmit({
				url : urlAddress,
...
<body>
	<div class="modal-body" style="height: 380px;">
		<form id="editForm" class="form-horizontal no-margin">
			<input type="hidden" id="hidId" name="id" value="${editCustomer.id}">
			<div class="form-group">
				<label for="no" class="col-sm-3 col-xs-3 control-label">客户编号</label>
				<div class="col-sm-8 col-xs-8">
					<input id="no" name="no" value="${editCustomer.no}"
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="name" name="name" value="${editCustomer.name}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="phone" name="phone" value="${editCustomer.phone}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="petno" name="petno" value="${editCustomer.petno}"
						class="form-control" type="text" />
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-8">
					<input id="remark" name="remark" value="${editCustomer.remark}"
						class="form-control" type="text" />
				</div>
			</div>
